...
namespace Streams.Resources
{
    public class ResourceReaderStream : Stream
    {
        ...
        {
            _key = key;
            _underlyingStream = new BufferedStream(stream, Size);
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            return CopyToBuffer(buffer, offset, count);
        }

        private int CopyToBuffer(byte[] buffer, int offset, int count)
        {
            int i = 0, j = 0;
        ...
        {
            // while not end of stream read next section key, compare with
            //required key and skip value if read key is not equal to required key
        ...
        private void Search(byte[] buff, int read)
        {
            var parts = Split(buff.Take(read).ToArray()).ToList();
            SearchThroughParts(read, parts);
        }

        private void SearchThroughParts(int read, List<byte[]> parts)
        {
        ...
        private void ValidateList(int read)
        {
            if (_found != null && read < Size && _found[_found.Count - 2] != 0 && _found[_found.Count - 1] != 1)
                _found = null;
        }

        private void CheckNonzeroBytes(List<byte[]> parts, int i)
        {
            if (i < parts.Count - 1)
            {
